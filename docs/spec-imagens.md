...
Decidido em 18/09/2026. Rigor alto: define forma de dado gravado, bucket e políticas. A migration é
...
- **Tipo de representação** por palavra: `photo`, `symbol`, `figure`, `action`, `none`. `none` é "não
...
até aprovação e créditos por tentativa; resultado em `modelos.md`. A tabela `image_attempts` é a
fonte.
